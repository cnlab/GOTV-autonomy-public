...
keyvars <- data_scored[c('vote_motive_autonomous', 
                         ...
                         'IAF_autonomous', 
                         'IAF_controlled')]
colnames(keyvars) <- c('AMV','CMV','AF','CF')
...
model_data <- model_data %>%
  mutate(motivation = case_when(
    term == "vote_motive_autonomous" ~ "AMV",
    term == "vote_motive_controlled" ~ "CMV"
  ))
...
# Intentions and behavior, bivariate association 

```{r}

bivariate_model = glmer(behavior_voting ~ vote_intention_vote +  (1 | study),
        data = filter(data_scaled, grepl("2020_student|2022_student|2024_student", study)), family = "binomial")

tab_model(bivariate_model, digits = 3, show.icc = FALSE, show.re.var = FALSE, show.stat = TRUE, show.df = TRUE, show.ngroups = FALSE)
...
model_data <- model_data %>%
  mutate(motivation = case_when(
    term == "IAF_autonomous" ~ "AF",
    term == "IAF_controlled" ~ "CF"
  ))
...
core_vars_toplot$variable <- recode(core_vars_toplot$variable, 
                                      "vote_intention_vote" = "Voting intention", 
                                      "CE_attitudes" = "Civic duty", 
                                      "CE_checklist" = "Civic behaviors")
...
emmeans::emtrends(sa1, ~ IAF_autonomous, var="vote_motive_autonomous", at = list(IAF_autonomous = c(-1, 0, 1))) %>% 
  data.frame() %>%
  mutate(`b [95% CI]` = sprintf("%.2f [%.2f, %.2f]", vote_motive_autonomous.trend, lower.CL, upper.CL)) %>% 
  select(IAF_autonomous, `b [95% CI]`)  



emmeans::emtrends(sa1, ~ IAF_controlled, var="vote_motive_controlled", at = list(IAF_controlled = c(-1, 0, 1))) %>% 
  data.frame() %>%
  mutate(`b [95% CI]` = sprintf("%.2f [%.2f, %.2f]", vote_motive_controlled.trend, lower.CL, upper.CL)) %>% 
  select(IAF_controlled, `b [95% CI]`)  
...
data_sem = data_scaled %>%
  rename("intention" = vote_intention_vote,
         "behavior" = behavior_voting)
  
# specify model
mediation_model = '

  # direct effects
  behavior ~ c1*vote_motive_autonomous + c2*vote_motive_controlled
  
  # mediator
  behavior ~ b*intention 
  intention ~ a1*vote_motive_autonomous + a2*vote_motive_controlled
...
'
# fit model
fit_mediation = lavaan::sem(mediation_model, data = data_sem, se = "boot")

# summarize model
summary(fit_mediation, standardized = TRUE, fit.measures=TRUE, ci = TRUE)
semboottools::standardizedSolution_boot(fit_mediation) %>%
...
```{r}
# plot model
lavaanPlot::lavaanPlot2(model = fit_mediation, #include = "covs",
                        stars = c("regress"), stand = TRUE,
                        coef_labels = TRUE)
```
