...
data_scored_ivs_long$scale <- recode(data_scored_ivs_long$scale, 
   "vote_motive_autonomous" = "Autonomous motivation to vote",
...
plotall_palette <- c("Autonomous motivation to vote" = "#0f758f", 
               "Autonomous functioning" = "#c0ebf7",
               "Controlled motivation to vote" = "#d34224",
               "Controlled functioning" = "#ffb5a6")


ggplot(aes(x = age, y = value, color = scale, fill = scale), data = data_scored_ivs_long) +
  geom_smooth(method = "lm", na.rm = TRUE) + 
  theme_minimal() + 
  labs(x = "Age", y = "Level Reported") +
  scale_colour_manual(values = plotall_palette) + 
  scale_fill_manual(values = plotall_palette) + 
...
# center at 18 and set to decades > 18
data_scaled_demos_clean$age_mod <- (as.numeric(data_scaled_demos_clean$age) - 18)/10


## MOTIVATION BY AGE 
vma_age <- lm(vote_motive_autonomous ~ age_mod, data = data_scaled_demos_clean)
vmc_age <- lm(vote_motive_controlled ~ age_mod, data = data_scaled_demos_clean)


tab_model(vma_age, vmc_age, show.icc = FALSE, show.re.var = FALSE, show.stat = TRUE, show.df = TRUE, df.method = "satterthwaite", show.ngroups = FALSE, dv.labels = c('VM-A', 'VM-C'), CSS = list(
            css.table = 'border-collapse: separate; width: 100%;',
            css.td = 'border-left: 2px solid lightgrey; padding: 4px;',
            css.td.first = 'border-left: none;',  # No border for the first cell of each model
            css.th = 'border-left: 2px solid lightgrey; padding: 4px;',
...
h1_age = lmerTest::lmer(vote_intention_vote ~ vote_motive_autonomous*age_mod + vote_motive_controlled*age_mod + (1 | study), data = data_scaled_demos_clean)

as.table(car::vif(h1_age, type = 'predictor'))


## CIVIC DUTY 

h3_age <- lmer(civic_duty ~ vote_motive_autonomous*age_mod + vote_motive_controlled*age_mod + (1 | study), data = data_scaled_demos_clean)

as.table(car::vif(h3_age, type = 'predictor'))


### table 

tab_model(h1_age, h3_age, digits = 3, show.icc = FALSE, show.re.var = FALSE, show.stat = TRUE, show.df = TRUE, df.method = "satterthwaite", show.ngroups = FALSE, dv.labels = c('H1: VM and voting intentions', 'H3: VM and sense of civic duty'), CSS = list(
            css.table = 'border-collapse: separate; width: 100%;',
            css.td = 'border-left: 2px solid lightgrey; padding: 4px;',
            css.td.first = 'border-left: none;',  # No border for the first cell of each model
            css.th = 'border-left: 2px solid lightgrey; padding: 4px;',
...
## IAF BY AGE 
iafa_age <- lmerTest::lmer(IAF_autonomous ~ age_mod + (1 | study), data = data_scaled_demos_clean)

iafc_age <- lmerTest::lmer(IAF_controlled ~ age_mod + (1 | study), data = data_scaled_demos_clean)

tab_model(iafa_age, iafc_age, show.icc = FALSE, show.re.var = FALSE, show.stat = TRUE, show.df = TRUE, df.method = "satterthwaite", show.ngroups = FALSE, dv.labels = c('IAF-A', 'IAF-C'), CSS = list(
            css.table = 'border-collapse: separate; width: 100%;',
            css.td = 'border-left: 2px solid lightgrey; padding: 4px;',
            css.td.first = 'border-left: none;',  # No border for the first cell of each model
            css.th = 'border-left: 2px solid lightgrey; padding: 4px;',
...
## INTENTIONS 
h5_age <- lmer(vote_intention_vote ~ IAF_autonomous*age_mod + IAF_controlled*age_mod + + (1 | study), data = filter(data_scaled_demos_clean, !grepl("2021_student", study)))

as.table(car::vif(h5_age, type = 'predictor'))


## CIVIC DUTY 
h7_age = lmer(civic_duty ~ IAF_autonomous*age_mod + IAF_controlled*age_mod + (1 | study), data = filter(data_scaled_demos_clean, !grepl("2021_student", study)))

as.table(car::vif(h7_age, type = 'predictor'))


### table 

tab_model(h5_age, h7_age, digits = 3, show.icc = FALSE, show.re.var = FALSE, show.stat = TRUE, show.df = TRUE, df.method = "satterthwaite", show.ngroups = FALSE, dv.labels = c('IAF and voting intentions', 'IAF and civic duty'), CSS = list(
            css.table = 'border-collapse: separate; width: 100%;',
            css.td = 'border-left: 2px solid lightgrey; padding: 4px;',
            css.td.first = 'border-left: none;',  # No border for the first cell of each model
            css.th = 'border-left: 2px solid lightgrey; padding: 4px;',
...
```{r}
# set x-axis points for age (centered at 18 and in decades)
vals = seq(0,7.5,1)

# intentions ~ age x motivation 
h1_age_pred <- ggeffects::ggpredict(h1_age, c("age_mod[vals]", "vote_motive_autonomous[-1,0,1]")) %>%
  data.frame() %>% 
  mutate(type = "Autonomous motivation to vote") %>% 
  bind_rows(ggeffects::ggpredict(h1_age, c("age_mod[vals]", "vote_motive_controlled[-1,0,1]")) %>% 
              data.frame() %>% 
              mutate(type = "Controlled motivation to vote")) %>% 
  mutate(x = (x *10) + 18) %>% 
  mutate(motivation_level = case_when(group == "-1" ~ "-1 SD", group == "0" ~ "0 (mean)", group == "1" ~ "+1 SD")) %>% 
  mutate(motivation_level = factor(motivation_level, levels = c("-1 SD", "0 (mean)", "+1 SD")))

ggplot(h1_age_pred, aes(x = x, y = predicted, color = motivation_level, fill = motivation_level)) + 
  geom_line() + 
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.2) + 
  theme_minimal() + 
  labs(x = "Age", y = "Predicted Voting Intention") + 
  facet_wrap(~type) 

# civic duty ~ age x motivation 

h3_age_pred <- ggeffects::ggpredict(h3_age, c("age_mod[vals]", "vote_motive_autonomous[-1,0,1]")) %>%
  data.frame() %>% 
  mutate(type = "Autonomous motivation to vote") %>% 
  bind_rows(ggeffects::ggpredict(h3_age, c("age_mod[vals]", "vote_motive_controlled[-1,0,1]")) %>% 
              data.frame() %>% 
              mutate(type = "Controlled motivation to vote")) %>% 
  mutate(x = (x *10) + 18) %>% 
  mutate(motivation_level = case_when(group == "-1" ~ "-1 SD", group == "0" ~ "0 (mean)", group == "1" ~ "+1 SD")) %>% 
...
ggplot(h3_age_pred, aes(x = x, y = predicted, color = motivation_level, fill = motivation_level)) + 
  geom_line() + 
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.2) + 
  theme_minimal() + 
  labs(x = "Age", y = "Predicted Civic Duty") + 
  facet_wrap(~type)
...
# intentions ~ age x IAF

h5_age_pred <- ggeffects::ggpredict(h5_age, c("age_mod[vals]", "IAF_autonomous[-1,0,1]")) %>%
  data.frame() %>% 
  mutate(type = "Autonomous functioning") %>% 
  bind_rows(ggeffects::ggpredict(h5_age, c("age_mod[vals]", "IAF_controlled[-1,0,1]")) %>% 
              data.frame() %>% 
              mutate(type = "Controlled functioning")) %>%
  mutate(x = (x *10) + 18) %>% 
  mutate(trait_level = case_when(group == "-1" ~ "-1 SD", group == "0" ~ "0 (mean)", group == "1" ~ "+1 SD")) %>% 
  mutate(trait_level = factor(trait_level, levels = c("-1 SD", "0 (mean)", "+1 SD")))


ggplot(h5_age_pred, aes(x = x, y = predicted, color = trait_level, fill = trait_level)) + 
  geom_line() + 
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.2) + 
  theme_minimal() + 
  labs(x = "Age", y = "Predicted Voting Intention") + 
...
# civic duty ~ age x IAF

h7_age_pred <- ggeffects::ggpredict(h7_age, c("age_mod[vals]", "IAF_autonomous[-1,0,1]")) %>%
  data.frame() %>% 
  mutate(type = "Autonomous functioning") %>% 
  bind_rows(ggeffects::ggpredict(h7_age, c("age_mod[vals]", "IAF_controlled[-1,0,1]")) %>% 
              data.frame() %>% 
              mutate(type = "Controlled functioning")) %>% 
  mutate(x = (x *10) + 18) %>% 
  mutate(trait_level = case_when(group == "-1" ~ "-1 SD", group == "0" ~ "0 (mean)", group == "1" ~ "+1 SD")) %>% 
...
ggplot(h7_age_pred, aes(x = x, y = predicted, color = trait_level, fill = trait_level)) + 
  geom_line() + 
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.2) + 
  theme_minimal() + 
  labs(x = "Age", y = "Predicted Civic Duty") + 
...
# political party identification interaction 
```{r}

h1_ext_exp = lmerTest::lmer(vote_intention_vote ~ vote_motive_autonomous*as.numeric(politics_party.y) + vote_motive_controlled*as.numeric(politics_party.y) + (1 | study), data = data_scaled_demos_clean)

tab_model(h1_ext_exp)


h2_ext_exp = glmer(behavior_voting ~ vote_motive_autonomous*as.numeric(politics_party.y)  + vote_motive_controlled*as.numeric(politics_party.y)  +  (1 | study),
        data = filter(data_scaled_demos_clean, grepl("2020_student|2022_student|2024_student", study)), family = "binomial")

tab_model(h2_ext_exp)


h5_ext_exp <- lmer(vote_intention_vote ~ IAF_autonomous*as.numeric(politics_party.y) + IAF_controlled*as.numeric(politics_party.y) + (1 | study), data = filter(data_scaled_demos_clean, !grepl("2021_student", study)))

tab_model(h5_ext_exp)


h6_ext_exp = glmer(behavior_voting ~ IAF_autonomous*as.numeric(politics_party.y) + IAF_controlled*as.numeric(politics_party.y) + (1 | study),
        data = filter(data_scaled_demos_clean, grepl("2020_student|2022_student|2024_student", study)), family = "binomial")

tab_model(h6_ext_exp)
